...
byte bcdToDec(byte val)
{
  ...
  LiquidCrystal_I2C lcd(0x3F, 2, 1, 0, 4, 5, 6, 7, 3, POSITIVE);  // Enter the i2C address here lcd(_x__  e.g. lcd(0x27
...
****************************************************************************************************/
  ...
  float Amps_Charging_Setting     = 0.12;                        // Set value at which charge is detected in Amps (normally between 0.5 and 0.8)  0.5 = my 330/LAM   0.8 = my LM
  int   Volts_R2_Value            = 7100;                       // Set R2 vaklue in mower settings (usually between 6000 and 7500)  7100 = my 330   7500 = my LM   7000 = my LAM
  ...
  // Aerator Drill Setup
  int   Drill_Spacing             = 15;                          // Cycle Loops between each drill cycle
...
if  (Mower_Running == 1)                                                                                                  Running_Test_for_Boundary_Wire();   // Test is the boundary wire is live
...
if ((Mower_Running == 1) && (GPS_Enabled == 1) && (GPS_Type == 1))                                                        Check_GPS_In_Out();                 // Test is the GPS Fence has been crossed
...
if ((Mower_Running == 1) && (Wire_Detected == 1) && (Outside_Wire == 0) && (Sonar_Hit == 0))                              Manouver_Mow_Aerate_The_Grass();    // Inputs to the wheel motors / blade motors according to surroundings
